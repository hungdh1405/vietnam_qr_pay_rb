...
module VietnamQrPay
  ...
  module CRC16
    ...
    # Computes the CRC over the UTF-8 bytes of the payload prefix.
    def crc16ccitt(content)
      content.encode(Encoding::UTF_8).bytes.reduce(0xFFFF) do |crc, byte|
        (TABLE[((crc >> 8) ^ byte) & 0xFF] ^ (crc << 8)) & 0xFFFF
      end
    end
  end
end
